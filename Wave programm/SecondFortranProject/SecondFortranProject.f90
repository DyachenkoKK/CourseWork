    ...
    CONTAINS 
        ...
        complex*16 MakeP, sgm1, sgm2, delt
            MakeP = 2*ci*(-((alph**2) - 0.5d0 * kappa2) * exp(sgm1*currentZ) + sgm1 * sgm2 * exp(sgm2*currentZ)) / delt
            !MakeP = (-((alph**2) - 0.5d0 * kappa2) * exp(sgm1*currentZ) + sgm1 * sgm2 * exp(sgm2*currentZ)) / delt
        end function MakeP
        
        function MakeR(alpha, sgm1, kappa2, delt)
        real*8 alpha, kappa2
        complex*16 MakeR, sgm1, delt
            MakeR = 2*ci*sgm1 * (-((alph**2) - 0.5d0 * kappa2) * exp(sgm1*currentZ) + (alph**2) * exp(sgm2*currentZ) ) / delt
            !MakeR = sgm1 * (-((alph**2) - 0.5d0 * kappa2) * exp(sgm1*currentZ) + (alph**2) * exp(sgm2*currentZ) ) / delt
        end function MakeR
        ...
        complex*16 MakeM, sgm2, delt
            if(alpha == 0 ) then 
                ...
                MakeM = 2*sgm2* (-exp(sgm1*currentZ) + (1 - 0.5d0 * kappa2 / (alph**2)) * exp(sgm2*currentZ)) / delt 
                !MakeM = -ci*sgm2* (-exp(sgm1*currentZ) + (1 - 0.5d0 * kappa2 / (alph**2)) * exp(sgm2*currentZ)) / delt 
            endif 
        end function MakeM
        ...
        complex*16 MakeS, sgm1, sgm2, delt
            MakeS = 2* (-sgm1 * sgm2 * exp(sgm1*currentZ) + ((alph**2) - 0.5d0 * kappa2) * exp(sgm2*currentZ) ) / delt
            !MakeS = -ci* (-sgm1 * sgm2 * exp(sgm1*currentZ) + ((alph**2) - 0.5d0 * kappa2) * exp(sgm2*currentZ) ) / delt
        end function MakeS
        ...
        integer i, j
            ...
            do i = 1, pointsNumber
                ...
                currentZ=z(i)
                call K(alph)
                tmp1(i)=k_arr(1,2) * q(2)
                call K(-alph)
                tmp2(i)=k_arr(1,2) * q(2)
            enddo
        ...
        complex*16 P, R, M, S
            ...
            delt = 4*ci* myu * (-(((alpha**2) - 0.5d0 * kappa2)**2) + (alpha**2) * sgm1 * sgm2)
